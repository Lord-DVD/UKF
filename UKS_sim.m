...
parameters = [3, 0.04, 0.3, -0.6];
% and the stock drift
mu = 0.05;
% plus a vector of log-stock prices, Stock
data = readtable("SimulatedHestonModel.csv");
...
xa = [x, zeros(1,2)];
Pa = diag(ones(1,3));
Pa(1,1) = 0.1;%sqrt(data.Volatility(1));
...
writematrix(transpose(scaled_vol),"UKF_Vol_1.csv");
smoothed_vol = smoothdata(scaled_vol,"gaussian",5);
writematrix(transpose(smoothed_vol),"UKF_Vol_2.csv");